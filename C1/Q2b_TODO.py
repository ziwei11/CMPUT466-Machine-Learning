...
def train(X_train, y_train, X_val, y_val, decay):
    ...
    for epoch in range(MaxIter):
        ...
        for b in range( int(np.ceil(N_train/batch_size)) ):
            ...
            w = w - alpha * ((1 / len(y_batch)) * np.dot(np.transpose(X_batch), (y_hat_batch - y_batch)) + decay * w)
...
best_decay = 0
for i in decay:    
    losses_train, risks_val, epoch_best, risk_best, w_best = train(X_train, y_train, X_val, y_val, i)
    if min(losses_train) < 100:
        best_decay = i
losses_train, risks_val, epoch_best, risk_best, w_best = train(X_train, y_train, X_val, y_val, best_decay)
y_hat, loss_test, risk_test = predict(X_test, w_best, y = y_test)
...
print("The best hyperparameter of decay: ", best_decay)
